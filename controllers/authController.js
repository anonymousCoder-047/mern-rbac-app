...
app.post('/signup', validateSignup, async (req, res) => {
    try {
        ...
        const _existing_usr = await get_users({ email: user_data.email });;
...
app.post('/forgot-password', forgotPasswordValidator, async (req, res) => {
    try {
        ...
        if(!_.isEmpty(_otp)) {
            ...
            const _mail_info = await send_mail("forgot_password", emailData, mail_username, _existing_usr?.email, "Reset Password", _attachments)
...
app.post('/verify-otp', validateOTPData, async (req, res) => {
    try {
        ...
        if(_otp_validation.status != 200) return apiResponse.ErrorResponse(res, "Sorry! incorrect OTP.");
        const _usr = await get_user_by_id(_otp_validation?.data?.user_id);
        
        return apiResponse.successResponseWithData(res, "OTP Verified Successfully", { email: _usr?.email });
    } catch(ex) {
...
app.post('/reset-password', resetPasswordValidator, async (req, res) => {
    try {
        let user_data = req.body;
        const [_existing_usr] = user_data?.email ? await get_users({ email: user_data?.email }) : [{}];
        const hashedPassword = await bcrypt.hash(user_data.password, 10);
        ...
        let _updated_usr = await update_user(_existing_usr?._id, { ...user_data, password: hashedPassword, username: _existing_usr?.username, email: _existing_usr?.email });
