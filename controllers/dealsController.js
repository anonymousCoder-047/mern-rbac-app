...
const { create, createMany, delete_deals, get_deals, get_deals_by_id, update_deals, delete_Many } = require('../services/dealsServices');
...
app.post('/bulk', canCreate('create'), upload.single('deals'), async (req, res) => {
    try {
        ...
        const _deals_data = [...new Set(sheetData?.map((x, idx) => ({ 
            ...
            sr_type: x?.['SR Type'],
            product_deal: _products?.find((_x) => _x?.product_name == x?.['Product Deals'])?._id,
    ...
    try {
        ...
        if(!_.isEmpty(deals_data)) {
            ...
            if(_.isEmpty(_existing_deals)) { 
                ...
                deals_data['profileId'] = profile_data?._id;
                deals_data['groupId'] = profile_data?.groupId?._id;
...
app.post('/bulk-delete', canDelete('delete'), async (req, res) => {
    try {
        ...
        if(!_.isEmpty(deals_data)) {
            const _deleted_deal = await delete_Many({ _id: { $in: deals_data?.ids?.map(id => ObjectId.createFromHexString(id)) }});
            if(!_.isEmpty(_deleted_deal)) return apiResponse.successResponseWithData(res, "Deals Deleted Successfully.", _deleted_deal);
            else apiResponse.ErrorResponse(res, "Unable to delete deals.");
        } else return apiResponse.badRequestResponse(res, "Sorry, missing field in body ", deals_data);
    } catch(err) {
        console.log("Internal server error: ", err);
