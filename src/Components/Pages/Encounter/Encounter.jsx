...
const Encounter = ({ permissions, elemKey, isAdmin }) => {
    ...
    const fetchEncounterData = async () => {
      try {
        const { Encounter } = endpoints;
        const { status, data } = await authenticatedServer.get(Encounter.view);

        if(status == 200) {
          setRows(data?.data)
        } else dispatchError(show_toast({ show: true, message: "No encounter found.", severity: "warning" }));
      } catch(error) {
    ...
    const handleClose = async (e, _action="") => {
      ...
      if(_action == 'edit') {
        setFormState({ 
          ...formState, 
          edit: true,
          enc_id: selectedRow?.enc_id,
          enc_data: selectedRow?.enc_data,
          sections: selectedRow?.sections,
          encounterId: selectedRow?._id,
        });
        ...
        const { Encounter } = endpoints;
        const { data } = await authenticatedServer.post(Encounter.delete, { id: selectedRow?.encounterId });

        if(data) dispatchError(show_toast({ show: true, message: "encounter deleted successfully ", severity: "success" }));
    ...
    const handleFormSubmit = async (e) => {
      try {
        ...
        if(validateForm(formState?.errors)) {
          if(formState?.encounterId == "") {
            let _formData = { ...formState };
            const { status, data } = await authenticatedServer.post(Encounter.create, _.pick(_formData, ['enc_id', 'sections']));
            
            if(status == 200) _formData = { ..._formData, encounterId: data?.data?._id, loading: false, disabled: false };
            else dispatchError(show_toast({ show: true, message: "Failed to save encounter.", severity: "warning" }));

            setFormState(_formData);
          } else {
            const _formState = { ...formState }
            const { status } = await authenticatedServer.patch(Encounter.patch + '/' + _formState?.encounterId, _.pick(_formState, ['enc_id', 'sections']))
                    ...
                    <Box sx={{ width: '100%', ...modalStyles }}>
                      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                        <Tabs value={value} onChange={handleChange} aria-label="basic tabs example">
                          <Tab label="Encounter details" {...a11yProps(0)} />
                        </Tabs>
                      </Box>
                      <CustomTabPanel value={value} index={0}>
                        <FormGroup>
                          <Box sx={{ p: 1 }}>
                            <InputLabel>
                              <Typography>Encounter Id</Typography>
                            </InputLabel>
                            <TextField 
                              required
                              type='text'
                              name='enc_id'
                              fullWidth
                              value={formState?.enc_id}
                              onChange={handleInputChange}
                              onBlur={handleInputBlur}
                              error={formState?.errors?.enc_id ? true : false}
                              helpertext="true"
                              helperText={formState?.errors?.enc_id ? "Encounter id is required" : false}
                            ...
                            <Select 
                              ...
                              >
                              {
                                  ...
                                  sections?.map((options) => (
                                      <MenuItem key={options?.section_name + "_section_" + options?.section_name + "-unique-" + Math.round(Math.random(1000), 2)} value={options?._id}>
